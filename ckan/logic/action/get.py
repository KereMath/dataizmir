...
def _group_or_org_list(context, data_dict, is_org=False):
    ...
    if sort_info and sort_info[0][0] == 'package_count':
        ...
        if sort == 'package_count desc':
            query = query.outerjoin(model.Member, model.Member.group_id == model.Group.id) \
                .outerjoin(model.Package, and_(
                ...
                .order_by(sqlalchemy.func.count(model.Package.id).desc())
        else:
            query = query.outerjoin(model.Member, model.Member.group_id == model.Group.id) \
                .outerjoin(model.Package, and_(
                    model.Member.table_id == model.Package.id,
                ...
                .filter(model.Group.is_organization == is_org) \
                .group_by(model.Group.id, model.Group.name) \
                .order_by(sqlalchemy.func.count(model.Package.id).asc())
...
def group_list_for_user(context, data_dict):
    ...
    if authz.is_sysadmin(user):
        ...
        groups_and_capacities = [(g, 'admin') for g in q.all()]

    # -------- Normal kullanıcı --------
    else:
        perm = data_dict.get('permission', 'read')
        roles = authz.get_roles_with_permission(perm)
        if not roles:
            return []
        ...
        q = model.Session.query(m, g).join(g, g.id == m.group_id).filter(
            m.table_name == 'user',
            m.table_id == user_obj.id,
            m.capacity.in_(roles),
            m.state == 'active',
            g.is_organization == False,
            g.state == 'active'
        )

        groups_and_capacities = [(group, member.capacity) for member, group in q.all()]
...
def package_autocomplete(context, data_dict):
    ...
    Veri setleri with names or titles that contain the query string will be
    returned.
    ...
    try:
        ...
        for cat in categories:
            ...
            result.append({
                ...
                'color': cat.color,
                'icon': cat.icon,
                'background_image': cat.background_image, # EKLENDİ
...
def theme_category_show(context, data_dict):
    ...
    from ckan.model.theme import ThemeCategory, DatasetThemeAssignment
    from ckan.model import Session
    from ckan.logic import ValidationError, NotFound, logic # Ensure logic is imported for get_action
    ...
    try:
        ...
        for dataset_id in dataset_ids:
            try:
                # Use context for package_show as well
            ...
            except Exception as dataset_e: # Catch specific dataset retrieval errors
                log.warning(f"Could not retrieve dataset {dataset_id} for theme {slug}: {dataset_e}")
                continue
        ...
        return {
            'category': {
                ...
                'icon': category.icon,
                'background_image': category.background_image, # EKLENDİ
...
def get_dataset_theme(context, data_dict):
    ...
    try:
        ...
        return {
            ...
            'color': theme.color,
            'icon': theme.icon,
            'background_image': theme.background_image, # EKLENDİ
