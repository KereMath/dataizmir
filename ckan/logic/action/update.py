...
def config_option_update(context, data_dict):
    ...
    * The ``app_globals`` (``g``) object is updated (this only happens for
    ...
    # PDF UPLOAD İŞLEVSELLİĞİ - BAŞLANGIC
    try:
        # PDF upload'ı kontrol et ve işle
        pdf_data = {}
        pdf_fields = [
            'data_policy_upload', 'kvkk_upload', 'terms_of_use_upload',
            'clear_data_policy_upload', 'clear_kvkk_upload', 'clear_terms_of_use_upload'
        ]
        
        # PDF ile ilgili field'ları topla
        for field in pdf_fields:
            if field in data_dict:
                pdf_data[field] = data_dict[field]
        
        if pdf_data:
            log.info('PDF upload fields found: {0}'.format(list(pdf_data.keys())))
            
            # Upload_pdfs fonksiyonunu import et ve çağır
            import os
            import mimetypes
            from ckan.lib.uploader import get_storage_path
            from werkzeug.datastructures import FileStorage
            
            # PDF upload fonksiyonunu burada tanımla
            def process_pdf_uploads(context, data_dict):
                # CKAN public dizinini kullan
                ckan_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
                ...
                log.info('PDF upload path: {0}'.format(public_path))
                ...
                for pdf_type, filename in pdf_types.items():
                    file_key = '{0}_upload'.format(pdf_type)
                    clear_key = 'clear_{0}_upload'.format(pdf_type)
                    ...
                    if data_dict.get(clear_key):
                        ...
                        if os.path.exists(file_path):
                            try:
                                os.remove(file_path)
                                log.info('PDF silindi: {0}'.format(file_path))
                                result[pdf_type] = {'deleted': True, 'filename': filename}
                            except Exception as e:
                                log.error('PDF silinirken hata: {0} - {1}'.format(filename, str(e)))
                                result[pdf_type] = {'error': 'Dosya silinirken hata: {0}'.format(str(e))}
                        continue
                    ...
                    try:
                        ...
                        log.info('Dosya kaydediliyor: {0}'.format(file_path))
                        ...
                        result[pdf_type] = {
                            ...
                            'size': file_size,
                            'path': file_path,
                            'url': h.url_for_static('/{0}'.format(filename), qualified=True)
                        }
                        
                        log.info('PDF başarıyla yüklendi: {0} ({1} bytes)'.format(file_path, file_size))
                        
                    except Exception as e:
                        log.error('PDF yüklenirken hata: {0} - {1}'.format(filename, str(e)))
                        result[pdf_type] = {'error': 'Dosya yüklenirken hata: {0}'.format(str(e))}
            ...
            # PDF upload'ları işle
            pdf_result = process_pdf_uploads(context, pdf_data)
            log.info('PDF upload result: {0}'.format(pdf_result))
            
    except Exception as e:
        log.error('PDF upload error: {0}'.format(str(e)))
        # PDF hatası config güncellemesini engellemesin
        pass
    # PDF UPLOAD İŞLEVSELLİĞİ - SON
